...
test("onOpenGithubClick", () => {
  expect(actions.onOpenGithubClick()).toEqual({
...
test("onRssListModalClick", () => {
  expect(actions.onRssListModalClick()).toEqual({
    type: actionTypes.RSSLISTMODAL
  });
});

test("onRssListDeleteClick", () => {
  expect(actions.onRssListDeleteClick("name", "url")).toEqual({
...
test("onRssListURLClick", () => {
  expect(actions.onRssListURLClick("url")).toEqual({
...
test("loadingItemList", () => {
  expect(actions.loadingItemList()).toEqual({
    type: actionTypes.ITEMLISTLOADING
  });
});

test("onSettingsModalClick", () => {
  expect(actions.onSettingsModalClick()).toEqual({
...
test("setSettings", () => {
  ...
  expect(actions.setSettings(updateDuration)).toEqual({
...
describe("onRssInputClick", () => {
  test("fetch失敗", () => {
    ...
    return store
      .dispatch(actions.onRssInputClick("name1", "NG"))
      .then(receiveActions => {
  ...
  test("fetch成功-parse失敗", () => {
    ...
    return store
      .dispatch(actions.onRssInputClick("name1", "http://example.com/NG"))
      .then(receiveActions => {
        // return of async actions
        expect(receiveActions).toEqual(expectedActions);
      });
